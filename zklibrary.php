...
class ZKLibrary {
	...
	public function __construct($ip = '192.168.1.201', $port = '4730', $protocol = 'UDP')
	{
		if($ip != null)
		{
			$this->ip = $ip;
	...
	private function encodeTime($time)
{
    ...
    $year = (int)$arr[0];
    $month = (int)ltrim($arr[1], '0');
    $day = (int)ltrim($arr[2], '0');
    $hour = (int)ltrim($arr[3], '0');
    $minute = (int)ltrim($arr[4], '0');
    $second = (int)ltrim($arr[5], '0');
